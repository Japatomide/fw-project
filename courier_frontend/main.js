...
async function sendFindInput() {
  const inputValue = document.getElementById("myInput").value;

  const url = `https://example.com/page?param=${encodeURIComponent(
    inputValue
  )}`;
...
function sendValue() {
  ...
  const url = `https://example.com/page?param=${encodeURIComponent(inputValue)}`;
